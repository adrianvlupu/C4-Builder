  ...
  if (!program.config) {
    ...
    if (program.watch) {
      ...
      if (options.GENERATE_PDF || options.GENERATE_COMPLETE_PDF_FILE || options.GENERATE_LOCAL_IMAGES) {
        ...
        console.log(
          chalk.bold(chalk.yellow('Rebuilding with pdf or local image generation enabled will take a long time'))
        );
      }
      ...
      watch(options.ROOT_FOLDER, { recursive: true }, async (evt, name) => {
        ...
        if (isBuilding) {
          ...
          if (options.GENERATE_PDF || options.GENERATE_COMPLETE_PDF_FILE || options.GENERATE_LOCAL_IMAGES)
            console.log(
              chalk.bold(chalk.yellow('Build already in progress, consider disabling pdf or local image generation '))
            );
